  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.user && result.user.isadmin) {
        setAuth({ isAuthenticated: true, user: result.user, isadmin: result.user.isadmin, superUser: result.superUser });
        navigate('/adminDash');
      } else if (result.user && result.user.superUser) {
        setAuth({ isAuthenticated: true, user: result.user, isadmin: result.user.isadmin, superUser: result.user.superUser });
        navigate('/super');
      } else if(result.user ){
        setAuth({ isAuthenticated: true, user: result.user, isadmin: result.user.isadmin, superUser: result.superUser });
        navigate('/');
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#F5F5F5]">
      <div className="bg-white border-2 border-[#5D3891] rounded-lg shadow-lg p-8 max-w-md w-full text-center">
        ...
        <p className="mt-4 text-[#5D3891]">
          Don't have an account? <Link to="/register" className="text-[#F99417]">Register</Link>
        </p>
      </div>
    </div>
  );
