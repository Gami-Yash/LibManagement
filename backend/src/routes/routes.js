  ...
  try {
    if (username.includes('@admin.com')) {
      ...
      if(password === user.password){
        const token = jwt.sign({ userId: user._id, userName: user.username, isadmin: user.isAdmin, superUser: user.superUser }, secretKey, { expiresIn: '1h' });
...
router.post('/borrow/:bookId', async (req, res) => {
  try {
    ...
    const confirmationMailOptions = {
      ...
      text: `Dear ${user.name},\n\nYou have successfully borrowed the book "${book.title}". It is due on ${dueDate.toLocaleString()}.\n\nThank you,\nLibrary Team`
    };
...
router.post('/createBook', async (req, res) => {
  ...
  try {
    ...
    const newBook = new Book({
      ...
      year: bookData.publishedDate ? new Date(bookData.publishedDate).getFullYear() : null,
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        res.status(200).json({ message: 'Book added successfully', bookData: savedBook });
      }
    });
  ...
  try {
    ...
    const booksWithImages = await Promise.all(books.map(async (book) => {
      try {
        ...
        const thumbnailUrl = data.items[0].volumeInfo.imageLinks?.smallThumbnail;
...
router.get('/displayBook/:id', async (req, res) => {
  ...
  try {
    ...
    const thumbnailUrl = bookData.imageLinks?.thumbnail || 'No thumbnail available';
...
router.post('/queryBook', async (req, res) => {
  ...
  try {
    ...
    const booksWithThumbnails = await Promise.all(booksFromDB.map(async (book) => {
      try {
        ...
        console.error(`Failed to fetch thumbnail for book ${book.title}:`, error.message);
        return {
          ...book.toObject(),
          smallThumbnail: null,
        };
...
router.post('/userInfo', async (req, res) => {
  ...
  try {
    ...
    res.json({ isAuthenticated: true, username: user.username, email: user.email });
  } catch (err) {
...
router.get('/admin/stats', async (req, res) => {
  try {
    ...
    const booksBorrowed = await Borrow.countDocuments({ returnDate: { $gt: new Date() } });
    ...
    const mostBorrowedBook = await Borrow.aggregate([
      ...
      { $sort: { count: -1 } },
      { $limit: 1 },
      { $lookup: { from: "books", localField: "_id", foreignField: "ISBN", as: "bookDetails" } },
      { $unwind: "$bookDetails" },
    ...
    const recentBorrows = await Borrow.countDocuments({ borrowdate: { $gte: sevenDaysAgo } });
